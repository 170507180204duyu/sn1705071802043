...
public class Ch16Activity1 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //实例化stateCallback,当打开相机时执行的方法（便于我们进行会话的创建）
        stateCallback=new CameraDevice.StateCallback() {
            ...
            public void onOpened(@NonNull CameraDevice cameraDevice) {
                ...
                //准备预览时使用的组件
                Surface surface=new Surface(surfaceTexture);//显示实况图像
                try {
    ...
    private void openCamera(int width,int height){
        ...
        try {
            cameraManager.openCamera("0",stateCallback,null);//0代表后置摄像头，1代表前置的,第二个参数当真正打开相机之后会执行回调方法
        }catch (Exception e){
            Log.e(Ch16Activity1.class.toString(),e.toString());
        }
    ...
    private void setCameraLayout(){
        ...
        //当 textureView准备好之后，自动调用getSurfaceTextureListener监听器。
        //用内部匿名类实现监听器
    ...
    private void sendSms(){
        ...
        smsManager.sendTextMessage("15169419141","15066707876","我的上课练习不用管",null,null);
    }
    private void callPhone(){
        Intent intent=new Intent(Intent.ACTION_CALL, Uri.parse("tel://15169419141"));//隐式启动
